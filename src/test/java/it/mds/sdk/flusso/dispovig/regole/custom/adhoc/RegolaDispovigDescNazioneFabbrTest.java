...
class RegolaDispovigDescNazioneFabbrTest {
    @Mock
    ...
    RegolaDispovigDescNazioneFabbr regola;
    ...
    void init(){
        ...
        regola = spy(new RegolaDispovigDescNazioneFabbr("TEST", "TEST", "TEST", parametriTest));
    }
    @AfterEach
    void close(){
        utilities.close();
    ...
    void costruttoreVuoto() {
        RegolaDispovigDescNazioneFabbr regola = new RegolaDispovigDescNazioneFabbr();
        assertTrue(regola instanceof RegolaDispovigDescNazioneFabbr);
    }
    ...
    void validaKO() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_NAZIONE_DESC"),anyString())).thenReturn(lista);
