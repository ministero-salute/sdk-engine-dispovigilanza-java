...
class RegolaDispovigLuogoIncidenteTest {
    @Mock
    ...
    RegolaDispovigLuogoIncidente regola;
    ...
    void init(){
        ...
        regola = spy(new RegolaDispovigLuogoIncidente("TEST", "TEST", "TEST", parametriTest));
    }
    ...
    void costruttoreVuoto() {
        RegolaDispovigLuogoIncidente regola = new RegolaDispovigLuogoIncidente();
        assertTrue(regola instanceof RegolaDispovigLuogoIncidente);
    }
    ...
    void validaKO() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        Mockito.when(recordMockito.getCampo("rapporto_luogoEpisodio_nomeStruttura")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_luogoEpisodio_codiceStruttura")).thenReturn("1");

        List<String> lista = new ArrayList<>();lista.add("001001");
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_LUOGHI_INCI"),anyString())).thenReturn(lista);
    ...
    void validaKO2() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        Mockito.when(recordMockito.getCampo("rapporto_luogoEpisodio_nomeStruttura")).thenReturn("nome struttura");
        Mockito.when(recordMockito.getCampo("rapporto_luogoEpisodio_codiceStruttura")).thenReturn("120201");
