...
class RegolaDispovigDescProvinciaFabbrTest {
    @Mock
    ...
    RegolaDispovigDescProvinciaFabbr regola;
    ...
    void init(){
        ...
        regola = spy(new RegolaDispovigDescProvinciaFabbr("TEST", "TEST", "TEST", parametriTest));
    }
    @AfterEach
    void close(){
        utilities.close();
    ...
    void costruttoreVuoto() {
        RegolaDispovigDescProvinciaFabbr regola = new RegolaDispovigDescProvinciaFabbr();
        assertTrue(regola instanceof RegolaDispovigDescProvinciaFabbr);
    }
    ...
    void validaOK() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_regioneFabbricante")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_provinciaFabbricante")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_descProvinciaFabbricante")).thenReturn("1");
        ...
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_PROVINCIA"),anyString())).thenReturn(lista2);
    ...
    void validaOK2() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_regioneFabbricante")).thenReturn("LAZIO");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_provinciaFabbricante")).thenReturn("RM");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_descProvinciaFabbricante")).thenReturn("ROMA");
