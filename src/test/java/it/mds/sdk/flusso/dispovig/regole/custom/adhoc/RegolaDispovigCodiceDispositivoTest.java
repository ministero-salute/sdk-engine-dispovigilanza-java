...
class RegolaDispovigCodiceDispositivoTest {
    @Mock
    ...
    RegolaDispovigCodiceDispositivo regola;
    ...
    void init(){
        ...
        regola = spy(new RegolaDispovigCodiceDispositivo("TEST", "TEST", "TEST", parametriTest));
    }
    @AfterEach
    void close(){
        utilities.close();
    ...
    void costruttoreVuoto() {
        RegolaDispovigCodiceDispositivo regola = new RegolaDispovigCodiceDispositivo();
        assertTrue(regola instanceof RegolaDispovigCodiceDispositivo);
    }
    ...
    void validaKO() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_tipoDispositivo")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_codiceDispositivo")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_codiceFabbrDispositivo")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_CND")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_desCND")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_tipoIVD")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_descTipoIVD")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_denominazioneFabbricante")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_nazioneFabbricante")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_denominazioneMandatario")).thenReturn("1");
        ...
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_DISP_MEDICI_GLOBAL"),anyString())).thenReturn(lista2);
