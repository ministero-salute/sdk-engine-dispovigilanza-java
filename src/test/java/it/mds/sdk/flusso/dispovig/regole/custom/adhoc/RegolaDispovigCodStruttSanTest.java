...
class RegolaDispovigCodStruttSanTest {
    @Mock
    ...
    RegolaDispovigCodStruttSan regola;
    ...
    void init(){
        ...
        regola = spy(new RegolaDispovigCodStruttSan("TEST", "TEST", "TEST", parametriTest));
    }

    @Test
    void costruttoreVuoto() {
        RegolaDispovigCodStruttSan regola = new RegolaDispovigCodStruttSan();
        assertTrue(regola instanceof RegolaDispovigCodStruttSan);
    }
    ...
    void validaOK() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        Mockito.when(recordMockito.getCampo("rapporto_compilatore_ruolo")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_compilatore_codStruttSanitaria")).thenReturn("1");
    ...
    void validaOK2() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        Mockito.when(recordMockito.getCampo("rapporto_compilatore_ruolo")).thenReturn("2");
        Mockito.when(recordMockito.getCampo("rapporto_compilatore_codStruttSanitaria")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_luogoEpisodio_codiceStruttura")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_luogoEpisodio_codAziendaOspLocale")).thenReturn("1");
