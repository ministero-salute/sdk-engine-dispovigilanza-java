...
class RegolaDispovigLuogoAziendaOspLocaleTest {
    @Mock
    ...
    RegolaDispovigLuogoAziendaOspLocale regola;
    ...
    void init(){
        ...
        regola = spy(new RegolaDispovigLuogoAziendaOspLocale("TEST", "TEST", "TEST", parametriTest));
    }
    @AfterEach
    void close(){
        utilities.close();
    ...
    void costruttoreVuoto() {
        RegolaDispovigLuogoAziendaOspLocale regola = new RegolaDispovigLuogoAziendaOspLocale();
        assertTrue(regola instanceof RegolaDispovigLuogoAziendaOspLocale);
    }
    ...
    void validaKO() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        Mockito.when(recordMockito.getCampo("rapporto_luogoEpisodio_codAziendaOspLocale")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_luogoEpisodio_aziendaOspLocale")).thenReturn("1");

        List<String> lista = new ArrayList<>();lista.add("123456");
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_STRUTT_COMPETENTI"),anyString())).thenReturn(lista);
    ...
    void validaKO2() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        Mockito.when(recordMockito.getCampo("rapporto_luogoEpisodio_codAziendaOspLocale")).thenReturn("201");
        Mockito.when(recordMockito.getCampo("rapporto_luogoEpisodio_aziendaOspLocale")).thenReturn("201");
