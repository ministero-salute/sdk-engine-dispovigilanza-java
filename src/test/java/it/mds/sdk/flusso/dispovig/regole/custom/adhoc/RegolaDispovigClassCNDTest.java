...
class RegolaDispovigClassCNDTest {
    @Mock
    ...
    void init(){
       ...
       // parametri.put("nomeTabella", "ANAG_EMUR_ESENZIONI");
        ...
        regola = spy(new RegolaDispovigClassCND("TEST", "TEST", "TEST", parametriTest));
    }
    @AfterEach
    void close(){
        utilities.close();
    ...
    void costruttoreVuoto() {
        RegolaDispovigClassCND regola = new RegolaDispovigClassCND();
        assertTrue(regola instanceof RegolaDispovigClassCND);
    }
    ...
    void validaKO() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        List<String> lista2 = new ArrayList<>();lista.add("001001");
        List<String> lista3 = new ArrayList<>();lista.add("001001");
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_DISP_MEDICI"),anyString())).thenReturn(lista);
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_CND_DISPO"),anyString())).thenReturn(lista2);
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_CND_TIPOLOGIA"),anyString())).thenReturn(lista3);
    ...
    void validaKO2() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_CND")).thenReturn("x");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_desCND")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_codiceDispositivo")).thenReturn("4610");
