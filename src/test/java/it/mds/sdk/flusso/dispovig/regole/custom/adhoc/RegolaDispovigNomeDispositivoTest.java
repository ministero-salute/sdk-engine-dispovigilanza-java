...
class RegolaDispovigNomeDispositivoTest {
    @Mock
    ...
    RegolaDispovigNomeDispositivo regola;
    ...
    void init(){
        ...
        regola = spy(new RegolaDispovigNomeDispositivo("TEST", "TEST", "TEST", parametriTest));
    }
    @AfterEach
    void close(){
        utilities.close();
    ...
    void costruttoreVuoto() {
        RegolaDispovigNomeDispositivo regola = new RegolaDispovigNomeDispositivo();
        assertTrue(regola instanceof RegolaDispovigNomeDispositivo);
    }
    ...
    void validaOK() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_DISP_MEDICI_DENOM"),anyString())).thenReturn(lista2);
