...
class RegolaDispovigTipoIVDTest {
    @Mock
    ...
    void init(){
        ...
        regola = spy(new RegolaDispovigTipoIVD("TEST", "TEST", "TEST", parametriTest));
    }
    @AfterEach
    void close(){
        utilities.close();
    ...
    void costruttoreVuoto() {
        RegolaDispovigTipoIVD regola = new RegolaDispovigTipoIVD();
        assertTrue(regola instanceof RegolaDispovigTipoIVD);
    }
    ...
    void validaKO() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_tipoDispositivo")).thenReturn("IVD");
        ...
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_CND_TIPOLOGIA"),anyString())).thenReturn(lista2);
    ...
    void validaKO2() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_codiceDispositivo")).thenReturn("1");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_tipoDispositivo")).thenReturn("MDD");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_tipoIVD")).thenReturn("1");
