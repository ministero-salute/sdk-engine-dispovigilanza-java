...
class RegolaDispovigDenomMandatarioTest {
    @Mock
    ...
    RegolaDispovigDenomMandatario regola;
    ...
    void init(){
        ...
        regola = spy(new RegolaDispovigDenomMandatario("TEST", "TEST", "TEST", parametriTest));
    }
    @AfterEach
    void close(){
        utilities.close();
    ...
    void costruttoreVuoto() {
        RegolaDispovigDenomMandatario regola = new RegolaDispovigDenomMandatario();
        assertTrue(regola instanceof RegolaDispovigDenomMandatario);
    }
    ...
    void validaKO2() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        List<String> lista2 = new ArrayList<>();lista2.add("1#1");
        List<String> lista3 = new ArrayList<>();lista3.add("123456");
        List<String> lista4 = new ArrayList<>();lista4.add("123456");
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_NAZIONE_NON_EU"),anyString())).thenReturn(lista);
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_NAZIONE_EU"),anyString())).thenReturn(lista2);
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_DISP_MEDICI"),anyString())).thenReturn(lista3);
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_DISPO_MEDICI_MAND"),anyString())).thenReturn(lista4);
    ...
    void validaKO() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_denominazioneMandatario")).thenReturn("denominazione");

        List<String> lista = new ArrayList<>();lista.add("PAKISTAN#PK");
        List<String> lista2 = new ArrayList<>();lista2.add("123456");
        List<String> lista3 = new ArrayList<>();lista3.add("123456");
        List<String> lista4 = new ArrayList<>();lista4.add("denominazione");
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_NAZIONE_NON_EU"),anyString())).thenReturn(lista);
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_NAZIONE_EU"),anyString())).thenReturn(lista2);
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_DISP_MEDICI"),anyString())).thenReturn(lista3);
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_DISPO_MEDICI_MAND"),anyString())).thenReturn(lista4);
