...
class RegolaDispovigDescRegioneFabbrTest {
    @Mock
    ...
    RegolaDispovigDescRegioneFabbr regola;
    ...
    void init(){
        ...
        regola = spy(new RegolaDispovigDescRegioneFabbr("TEST", "TEST", "TEST", parametriTest));
    }
    @AfterEach
    void close(){
        utilities.close();
    ...
    void costruttoreVuoto() {
        RegolaDispovigDescRegioneFabbr regola = new RegolaDispovigDescRegioneFabbr();
        assertTrue(regola instanceof RegolaDispovigDescRegioneFabbr);
    }
    ...
    void validaOK() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_descRegioneFabbricante")).thenReturn("1");

        List<String> lista = new ArrayList<>();lista.add("001001");
        List<String> lista2 = new ArrayList<>();lista.add("120");
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_DISP_MEDICI"),anyString())).thenReturn(lista);
        utilities.when(() -> Utilities.getValoriAnagrafica(eq("ANAG_DISPOVIG_REGIONE"),anyString())).thenReturn(lista2);
    ...
    void validaKO2() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_descRegioneFabbricante")).thenReturn("ROMA");
    ...
    void validaKO() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_nazioneFabbricante")).thenReturn("IT");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_regioneFabbricante")).thenReturn("120");
        Mockito.when(recordMockito.getCampo("rapporto_dispositivo_descRegioneFabbricante")).thenReturn("");
