...
@Slf4j
@NoArgsConstructor
@XmlDiscriminatorValue("RegolaDispovigDescNazioneMandatario")
public class RegolaDispovigDescNazioneMandatario extends RegolaGenerica {
	
	public RegolaDispovigDescNazioneMandatario(String nome, String codErrore, String desErrore, Parametri parametri) {
	...
	public List<Esito> valida(String nomeCampo, RecordDtoGenerico recordDtoGenerico) {
		...
		try {						
			...
			if (listaNaizoneNonUE.contains(rapporto_dispositivo_descNazioneFabbricante+"#"+rapporto_dispositivo_nazioneFabbricante)) {
				if (StringUtils.isBlank(rapporto_dispositivo_descNazioneMandatario)) {
					listaEsiti.add(creaEsitoKO(nomeCampo, codErr, desErr));
					return listaEsiti;
				}
				List<String> listaNazioniDesc = Utilities.getValoriAnagrafica("ANAG_DISPOVIG_NAZIONE_DESC",rapporto_dispositivo_descNazioneMandatario+"#"+rapporto_dispositivo_nazioneMandatario);
				
				if (!listaNazioniDesc.contains(rapporto_dispositivo_descNazioneMandatario+"#"+rapporto_dispositivo_nazioneMandatario)) {
			...
			listaEsiti.add(creaEsitoOk(nomeCampo));
				...
				log.error("Non è possibile validare la regola RegolaDispovigDescNazioneMandatario " + nomeCampo, e);
			throw new ValidazioneImpossibileException("Non è possibile validare la regola RegolaDispovigDescNazioneMandatario " + nomeCampo );
