...
@Slf4j
@NoArgsConstructor
@XmlDiscriminatorValue("RegolaDispovigRegioneFabbr")
public class RegolaDispovigRegioneFabbr extends RegolaGenerica {
	
	public RegolaDispovigRegioneFabbr(String nome, String codErrore, String desErrore, Parametri parametri) {
		...
		try {						
			...
			String rapporto_dispositivo_regioneFabbricante = (recordDtoGenerico
					.getCampo("rapporto_dispositivo_regioneFabbricante") != null
							...
							: null);
			String rapporto_dispositivo_descRegioneFabbricante = (recordDtoGenerico
					.getCampo("rapporto_dispositivo_descRegioneFabbricante") != null
							? (String) recordDtoGenerico.getCampo("rapporto_dispositivo_descRegioneFabbricante")
							: null);
			...
			String codErr3 = "10791";
			String desErr3 = "Codice Regione non congruente con la Nazione del Fabbricante";
			/*
			 * se fabbricante italiano allora deve essere valorizzato
			 */
			if ("IT".equalsIgnoreCase(rapporto_dispositivo_nazioneFabbricante)) {
				if (StringUtils.isBlank(rapporto_dispositivo_regioneFabbricante)) {	
				...
				if (!listaDispositiviMedici.contains(rapporto_dispositivo_codiceDispositivo)) {
					List<String> listaRegioniConDesc = Utilities.getValoriAnagrafica("ANAG_DISPOVIG_REGIONE",rapporto_dispositivo_regioneFabbricante+"#"+rapporto_dispositivo_descRegioneFabbricante);
					
					if (!listaRegioniConDesc.contains(rapporto_dispositivo_regioneFabbricante+"#"+rapporto_dispositivo_descRegioneFabbricante)) {
			...
			listaEsiti.add(creaEsitoOk(nomeCampo));
				...
				log.error("Non è possibile validare la regola RegolaDispovigRegioneFabbr " + nomeCampo, e);
			throw new ValidazioneImpossibileException("Non è possibile validare la regola RegolaDispovigRegioneFabbr " + nomeCampo );
