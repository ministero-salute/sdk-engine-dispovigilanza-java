...
@Slf4j
@NoArgsConstructor
@XmlDiscriminatorValue("RegolaDispovigDescNazioneFabbr")
public class RegolaDispovigDescNazioneFabbr extends RegolaGenerica {
	
	public RegolaDispovigDescNazioneFabbr(String nome, String codErrore, String desErrore, Parametri parametri) {
	...
	public List<Esito> valida(String nomeCampo, RecordDtoGenerico recordDtoGenerico) {
		...
		try {						
			...
			List<String> listaDescNazioni = Utilities.getValoriAnagrafica("ANAG_DISPOVIG_NAZIONE_DESC",rapporto_dispositivo_descNazioneFabbricante+"#"+rapporto_dispositivo_nazioneFabbricante);
			
			String codErr = "1078";
			String desErr = "Non appartenenza al dominio di riferimento";

				if (!listaDescNazioni.contains(rapporto_dispositivo_descNazioneFabbricante+"#"+rapporto_dispositivo_nazioneFabbricante)) {
			...
			listaEsiti.add(creaEsitoOk(nomeCampo));
				...
				log.error("Non è possibile validare la regola RegolaDispovigDescNazioneFabbr " + nomeCampo, e);
			throw new ValidazioneImpossibileException("Non è possibile validare la regola RegolaDispovigDescNazioneFabbr " + nomeCampo );
