...
@Slf4j
@NoArgsConstructor
@XmlDiscriminatorValue("RegolaDispovigDescComuneMandatario")
public class RegolaDispovigDescComuneMandatario extends RegolaGenerica {
	
	public RegolaDispovigDescComuneMandatario(String nome, String codErrore, String desErrore, Parametri parametri) {
		...
		try {						
			...
			String rapporto_dispositivo_provinciaMandatario = (recordDtoGenerico
					.getCampo("rapporto_dispositivo_provinciaMandatario") != null
							...
							: null);
			String rapporto_dispositivo_comuneMandatario = (recordDtoGenerico
					.getCampo("rapporto_dispositivo_comuneMandatario") != null
							? (String) recordDtoGenerico.getCampo("rapporto_dispositivo_comuneMandatario")
							: null);
			String rapporto_dispositivo_descComuneMandatario = (recordDtoGenerico
					.getCampo("rapporto_dispositivo_descComuneMandatario") != null
							? (String) recordDtoGenerico.getCampo("rapporto_dispositivo_descComuneMandatario")
							: null);
			...
			if ("IT".equalsIgnoreCase(rapporto_dispositivo_nazioneMandatario) &&
				...
				if (StringUtils.isBlank(rapporto_dispositivo_descComuneMandatario)) {	
				...
				if (!listaDispositiviMedici.contains(rapporto_dispositivo_codiceDispositivo)) {	
					List<String> listaComuni = Utilities.getValoriAnagrafica("ANAG_DISPOVIG_COMUNE",rapporto_dispositivo_comuneMandatario + "#" + rapporto_dispositivo_descComuneMandatario
							+ "#" + rapporto_dispositivo_provinciaMandatario);
					
					if (!listaComuni.contains(
							rapporto_dispositivo_comuneMandatario + "#" + rapporto_dispositivo_descComuneMandatario
							+ "#" + rapporto_dispositivo_provinciaMandatario)) {
					...
					/*
					 * TODO: codice dispositivo esiste - verificare che la desc comune del fabbricante sia uguale a quello presente sul db
					 * rapporto_dispositivo_comuneMandatario
					 */
				}
			}
			...
			listaEsiti.add(creaEsitoOk(nomeCampo));
				...
				log.error("Non è possibile validare la regola RegolaDispovigDescComuneMandatario " + nomeCampo, e);
			throw new ValidazioneImpossibileException("Non è possibile validare la regola RegolaDispovigDescComuneMandatario " + nomeCampo );
