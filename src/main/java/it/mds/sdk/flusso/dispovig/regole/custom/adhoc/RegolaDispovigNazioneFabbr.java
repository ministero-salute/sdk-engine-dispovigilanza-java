...
@Slf4j
@NoArgsConstructor
@XmlDiscriminatorValue("RegolaDispovigNazioneFabbr")
public class RegolaDispovigNazioneFabbr extends RegolaGenerica {
	
	public RegolaDispovigNazioneFabbr(String nome, String codErrore, String desErrore, Parametri parametri) {
	...
	public List<Esito> valida(String nomeCampo, RecordDtoGenerico recordDtoGenerico) {
		...
		try {						
			...
			if (!listaDispositiviMedici.contains(rapporto_dispositivo_codiceDispositivo)) {
				List<String> listaNazioni = Utilities.getValoriAnagrafica("ANAG_DISPOVIG_NAZIONE",rapporto_dispositivo_nazioneFabbricante);
				
				if (!listaNazioni.contains(rapporto_dispositivo_nazioneFabbricante)) {
			...
			listaEsiti.add(creaEsitoOk(nomeCampo));
				...
				log.error("Non è possibile validare la regola RegolaDispovigNazioneFabbr " + nomeCampo, e);
			throw new ValidazioneImpossibileException("Non è possibile validare la regola RegolaDispovigNazioneFabbr " + nomeCampo );
