		...
		try {
			...
			if(dataValidatoreString != null){
				if(dataDaComparareString!= null ) {
					...
					if(dataDaComparare.isBefore(dataValidatore) || dataDaComparare.isEqual(dataValidatore)){
						listaEsiti.add(creaEsitoOk(nomeCampo));
					}else{
						listaEsiti.add(creaEsitoKO(nomeCampo,this.getCodErrore(),this.getDesErrore()));
								//.withDescrizione("Il campo  " + nomeCampo + "deve essere antecedente al campo " + nomeCampoValidatore)
					}
				}else{
					listaEsiti.add(creaEsitoKO(nomeCampo,this.getCodErrore(),this.getDesErrore()));
				}
			...
			listaEsiti.add(creaEsitoKO(nomeCampo,this.getCodErrore(),"Formato data non corretto"));
		}
		return listaEsiti;
	}
}
