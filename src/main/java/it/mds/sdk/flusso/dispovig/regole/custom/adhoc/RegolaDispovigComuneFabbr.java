...
@Slf4j
@NoArgsConstructor
@XmlDiscriminatorValue("RegolaDispovigComuneFabbr")
public class RegolaDispovigComuneFabbr extends RegolaGenerica {
	
	public RegolaDispovigComuneFabbr(String nome, String codErrore, String desErrore, Parametri parametri) {
	...
	public List<Esito> valida(String nomeCampo, RecordDtoGenerico recordDtoGenerico) {
		...
		try {						
			...
			if ("IT".equalsIgnoreCase(rapporto_dispositivo_nazioneFabbricante)) {
				if (StringUtils.isBlank(rapporto_dispositivo_comuneFabbricante)) {
				...
				if (!listaDispositiviMedici.contains(rapporto_dispositivo_codiceDispositivo)) {
					...
					List<String> listaComuni = Utilities.getValoriAnagrafica("ANAG_DISPOVIG_COMUNE",rapporto_dispositivo_comuneFabbricante.substring(3,
							rapporto_dispositivo_comuneFabbricante.length()) + "#"
							+ rapporto_dispositivo_descComuneFabbricante + "#"
							+ rapporto_dispositivo_provinciaFabbricante);
					
					if (!listaComuni.contains(rapporto_dispositivo_comuneFabbricante.substring(3,
							rapporto_dispositivo_comuneFabbricante.length()) + "#"
							+ rapporto_dispositivo_descComuneFabbricante + "#"
							+ rapporto_dispositivo_provinciaFabbricante)) {
					 ...
					 * rapporto_dispositivo_comuneFabbricante
					 */
				}
			}
			...
			listaEsiti.add(creaEsitoOk(nomeCampo));
				...
				log.error("Non è possibile validare la regola RegolaDispovigComuneFabbr " + nomeCampo, e);
			throw new ValidazioneImpossibileException("Non è possibile validare la regola RegolaDispovigComuneFabbr " + nomeCampo );
