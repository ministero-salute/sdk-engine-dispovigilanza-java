...
@Slf4j
@NoArgsConstructor
@XmlDiscriminatorValue("RegolaDispovigTipoIVD")
public class RegolaDispovigTipoIVD extends RegolaGenerica {
	
	public RegolaDispovigTipoIVD(String nome, String codErrore, String desErrore, Parametri parametri) {
	...
	public List<Esito> valida(String nomeCampo, RecordDtoGenerico recordDtoGenerico) {
		...
		try {						
			...
			String rapporto_dispositivo_tipoIVD = (recordDtoGenerico
					.getCampo("rapporto_dispositivo_tipoIVD") != null
...
//			List<String> listaClassificazioneCND = Utilities.getValoriAnagrafica("ANAG_DISPOVIG_CND_DISPO");
			...
			if (!listaDispositiviMedici.contains(rapporto_dispositivo_codiceDispositivo)) {
				List<String> listaTipologiaCND = Utilities.getValoriAnagrafica("ANAG_DISPOVIG_CND_TIPOLOGIA","NO-IVD#" + rapporto_dispositivo_tipoIVD);
				
				if ("IVD".equalsIgnoreCase(rapporto_dispositivo_tipoDispositivo)) {
					if (!listaTipologiaCND.contains("IVD#" + rapporto_dispositivo_tipoIVD)) {
						listaEsiti.add(creaEsitoKO(nomeCampo, codErr, desErr));
						return listaEsiti;
					}
				}else if ("MDD".equalsIgnoreCase(rapporto_dispositivo_tipoDispositivo)) {
					if (!listaTipologiaCND.contains("NO-IVD#" + rapporto_dispositivo_tipoIVD)) {
						listaEsiti.add(creaEsitoKO(nomeCampo, codErr, desErr));
						return listaEsiti;
					}
				}
				...
				/*
				 * TODO: codice dispositivo esiste - verificare che il tipo IVD sia uguale a quello presente sul db
				 */
			}
			...
			listaEsiti.add(creaEsitoOk(nomeCampo));
				...
				log.error("Non è possibile validare la regola RegolaDispovigTipoIVD " + nomeCampo, e);
			throw new ValidazioneImpossibileException("Non è possibile validare la regola RegolaDispovigTipoIVD " + nomeCampo );
